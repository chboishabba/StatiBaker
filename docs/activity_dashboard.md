...
- OpenRecall activity indicators (app/title/device/preview with deep links back to OpenRecall)
...
- including `runs/<date>/logs/openrecall/<date>.jsonl` when the OpenRecall adapter is enabled
...
- kind (chat/shell/input/window/git/git_branch/pr/activity/openrecall)
...
OpenRecall rows:
- are a bounded personal observer lane, not canonical SB authority
- may include short preview text and explicit `open in OpenRecall` links
- must not copy screenshot bytes or full OCR bodies into dashboard payloads
...
- Real when provided: OpenRecall activity rows exported from a local `recall.db`.
...
"$SB/scripts/run_day.sh" "$DATE" "$REPO" \
  ...
  "/abs/path/openrecall/recall.db" \
  "http://127.0.0.1:8082" \
  "workstation-a" \
  "alice-home-2026-02-08"
...
Use positional arguments 26..29 for the OpenRecall adapter:
- arg 26: `OPENRECALL_DB_PATH`
- arg 27: `OPENRECALL_BASE_URL`
- arg 28: `OPENRECALL_DEVICE_ID`
- arg 29: `OPENRECALL_SESSION_ID`
