...
def normalize_record(record: Dict[str, Any], source: str) -> Dict[str, Any]:
    ...
    shaped["item_id"] = shaped.get("item_id") or shaped.get("media_id") or shaped.get("uri") or shaped.get("path")
    shaped["item_title"] = shaped.get("item_title") or shaped.get("title")
    shaped["app"] = shaped.get("app") or "vlc"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Normalize VLC history rows into media_consumption.")
    ...
    parser.add_argument("--source", default="vlc_history_stub", help="Provenance source label")
