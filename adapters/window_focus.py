...
def normalize_record(record: Dict[str, Any], source: str) -> Dict[str, Any]:
    ...
    title_hash = record.get("window_title_hash")
    if window_title and not title_hash:
        title_hash = sha256_text(window_title)

    normalized = {
        "ts": ts,
        "signal": "window_focus",
        "app_id": record.get("app_id") or record.get("app"),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Normalize window focus records (meta-only).")
    ...
    parser.add_argument("--source", default="window_focus", help="Provenance source label")
