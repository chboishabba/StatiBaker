...
def normalize_record(record: Dict[str, Any], source: str) -> Dict[str, Any]:
    ...
    tweet_id = record.get("tweet_id") or record.get("post_id")
    post_hash = record.get("post_id_hash") or (sha256_text(tweet_id) if tweet_id else None)

    handle = record.get("handle") or record.get("author")
    author_hash = record.get("author_hash") or (sha256_text(handle) if handle else None)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Normalize Twitter/X feed records (meta-only stub).")
    ...
    parser.add_argument("--source", default="twitter_stub", help="Provenance source label")
