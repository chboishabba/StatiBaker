...
def normalize_record(record: Dict[str, Any], source: str) -> Dict[str, Any]:
    ...
    status_id = record.get("status_id") or record.get("post_id")
    post_hash = record.get("post_id_hash") or (sha256_text(status_id) if status_id else None)

    acct = record.get("acct") or record.get("author")
    author_hash = record.get("author_hash") or (sha256_text(acct) if acct else None)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Normalize Mastodon feed records (meta-only stub).")
    ...
    parser.add_argument("--source", default="mastodon_stub", help="Provenance source label")
