...
def normalize_record(record: Dict[str, Any], source: str) -> Dict[str, Any]:
    ...
    message_id = record.get("message_id") or record.get("mid")
    post_hash = record.get("post_id_hash") or (sha256_text(message_id) if message_id else None)

    author = record.get("sender_id") or record.get("author")
    ...
    thread_hash = record.get("thread_id_hash") or (sha256_text(thread_id) if thread_id else None)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Normalize Facebook Messenger records (meta-only stub).")
    ...
    parser.add_argument("--source", default="facebook_messenger_stub", help="Provenance source label")
