...
def normalize_record(record: Dict[str, Any], source: str) -> Dict[str, Any]:
    ...
    message_id = record.get("message_id") or record.get("msg_id")
    post_hash = record.get("post_id_hash") or (sha256_text(message_id) if message_id else None)

    author = record.get("contact_id") or record.get("author")
    author_hash = record.get("author_hash") or (sha256_text(author) if author else None)

    chat_id = record.get("chat_id") or record.get("thread_id")
    thread_hash = record.get("thread_id_hash") or (sha256_text(chat_id) if chat_id else None)

    normalized = {
        "ts": ts,
        "signal": "social_feed",
        "platform": "whatsapp",
        "event_type": record.get("event_type") or record.get("action") or "event_observed",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Normalize WhatsApp records (meta-only stub).")
    ...
    parser.add_argument("--source", default="whatsapp_stub", help="Provenance source label")
