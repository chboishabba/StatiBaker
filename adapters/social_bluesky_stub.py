...
def normalize_record(record: Dict[str, Any], source: str) -> Dict[str, Any]:
    ...
    uri = record.get("uri") or record.get("post_id")
    post_hash = record.get("post_id_hash") or (sha256_text(uri) if uri else None)

    did = record.get("did") or record.get("author")
    author_hash = record.get("author_hash") or (sha256_text(did) if did else None)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Normalize Bluesky feed records (meta-only stub).")
    ...
    parser.add_argument("--source", default="bluesky_stub", help="Provenance source label")
